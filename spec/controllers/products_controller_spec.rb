...
RSpec.describe ProductsController, type: :controller do
  describe 'GET #index' do
    ...
    it 'assigns all products as @products' do
      user = FactoryBot.create(:user, username: 'tester1')
      category = FactoryBot.create(:category)
      product = FactoryBot.create(:product, user: user, category: category)
      
      get :index
      expect(assigns(:products)).to eq([product])
      expect(assigns(:products).first.user.username).to eq('tester1')
      expect(assigns(:products).first.category.name).to eq(category.name)
      expect(assigns(:products).count).to eq(1)
    end

    it 'render index template when there are not products' do
      get :index
